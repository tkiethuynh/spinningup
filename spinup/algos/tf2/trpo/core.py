...
class MLPCategoricalActor(tf.keras.Model):
    ...
    def kl_divergence(self, logits1, logits2):
        ...
        # For categorical: sum exp(logits1) * (logits1 - logits2)
        p = tf.nn.softmax(logits1)
        logp = tf.nn.log_softmax(logits1)
        logq = tf.nn.log_softmax(logits2)
        return tf.reduce_sum(p * (logp - logq), axis=1)
...
class MLPGaussianActor(tf.keras.Model):
    ...
    def kl_divergence(self, mu_std1, mu_std2):
        mu1, std1 = mu_std1
        mu2, std2 = mu_std2
        log_std1 = tf.math.log(std1)
        log_std2 = tf.math.log(std2)
        # KL(N1 || N2) = log(std2/std1) + (std1^2 + (mu1-mu2)^2)/(2*std2^2) - 0.5
        kl = log_std2 - log_std1 + (tf.square(std1) + tf.square(mu1 - mu2)) / (2.0 * tf.square(std2)) - 0.5
